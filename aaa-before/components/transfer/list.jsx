...
export default class TransferList extends React.Component {
    ...
    renderCheckbox({ prefixCls, filteredDataSource, checked, checkPart, disabled, checkable }) {
        ...
        return (<span ref="checkbox" className={checkboxCls} onClick={() => this.props.handleSelectAll(filteredDataSource, checkAll)}>
    ...
    render() {
        const { prefixCls, dataSource, titleText, filter, checkedKeys, body, footer, showSearch, render, style } = this.props;
        ...
        const showItems = dataSource.map(item => {
            ...
            return (<li onClick={() => this.handleSelect(item)} key={item.key} title={renderedText}>
        ...
        <div className={`${prefixCls}-header`}>
          {this.renderCheckbox({
            prefixCls: 'rubix-transfer',
            checked: checkStatus === 'all',
            checkPart: checkStatus === 'part',
            checkable: <span className={'rubix-transfer-checkbox-inner'}></span>,
            filteredDataSource,
            disabled: false,
        })}
        ...
        {bodyDom ||
            ...
            {showSearch ? <div className={`${prefixCls}-body-search-wrapper`}>
              <Search prefixCls={`${prefixCls}-search`} onChange={this.handleFilter} handleClear={this.handleClear} placeholder={searchPlaceholder || '请输入搜索内容'} value={filter}/>
            </div> : null}
            <Animate component="ul" transitionName={this.state.mounted ? `${prefixCls}-highlight` : ''} transitionLeave={false}>
              {showItems.length > 0
                ? showItems
