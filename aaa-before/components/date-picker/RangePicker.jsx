...
export default class RangePicker extends React.Component {
    ...
    render() {
        ...
        const { disabledDate, showTime, getCalendarContainer, transitionName, disabled, popupStyle, align, style, onOk } = this.props;
        ...
        const calendar = (<RangeCalendar prefixCls="rubix-calendar" formatter={props.getFormatter()} className={calendarClassName} timePicker={props.timePicker} disabledDate={disabledDate} dateInputPlaceholder={[startPlaceholder, endPlaceholder]} locale={locale.lang} onOk={onOk} defaultValue={[defaultCalendarValue, defaultCalendarValue]} {...calendarHandler}/>);
        const clearIcon = (!props.disabled && state.value && (state.value[0] || state.value[1]))
            ? <Icon type="cross-circle" className="rubix-calendar-picker-clear" onClick={this.clearSelection}/> : null;
        return (<span className={props.pickerClass} style={style}>
      <RcDatePicker formatter={props.getFormatter()} transitionName={transitionName} disabled={disabled} calendar={calendar} value={state.value} prefixCls="rubix-calendar-picker-container" style={popupStyle} align={align} getCalendarContainer={getCalendarContainer} onOpen={props.toggleOpen} onClose={props.toggleOpen} {...pickerChangeHandler}>
        {({ value }) => {
            const start = value[0];
            const end = value[1];
            return (<span className={props.pickerInputClass} disabled={disabled}>
                <input disabled={disabled} readOnly value={start ? props.getFormatter().format(start) : ''} placeholder={startPlaceholder} className="rubix-calendar-range-picker-input"/>
                <span className="rubix-calendar-range-picker-separator"> ~ </span>
                <input disabled={disabled} readOnly value={end ? props.getFormatter().format(end) : ''} placeholder={endPlaceholder} className="rubix-calendar-range-picker-input"/>
                {clearIcon}
