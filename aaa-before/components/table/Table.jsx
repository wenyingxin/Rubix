...
export default class Table extends React.Component {
    constructor(props) {
        ...
        this.renderSelectionRadio = (value, record, index) => {
        ...
        <Radio disabled={props.disabled} onChange={(e) => this.handleRadioSelect(record, rowIndex, e)} value={rowIndex} checked={checked}/>
      </span>);
        };
        this.renderSelectionCheckBox = (value, record, index) => {
            let rowIndex = this.getRecordKey(record, index); // 从 1 开始
            let checked;
            if (this.state.selectionDirty) {
                checked = this.state.selectedRowKeys.indexOf(rowIndex) >= 0;
            ...
            const props = this.getCheckboxPropsByItem(record);
            return (<span onClick={stopPropagation}>
        <Checkbox checked={checked} disabled={props.disabled} onChange={(e) => this.handleSelect(record, rowIndex, e)}/>
      </span>);
        };
        ...
        if (this.props.rowSelection) {
            ...
            if (!data.length) {
                ...
                checked = this.state.selectionDirty
                    ? data.every((item, i) => this.state.selectedRowKeys.indexOf(this.getRecordKey(item, i)) >= 0)
                    : (data.every((item, i) => this.state.selectedRowKeys.indexOf(this.getRecordKey(item, i)) >= 0) ||
            ...
            if (this.props.rowSelection.type === 'radio') {
                selectionColumn = {
                    ...
                    className: 'rubix-table-selection-column',
                };
            }
            else {
                const checkboxAllDisabled = data.every(item => this.getCheckboxPropsByItem(item).disabled);
                const checkboxAll = (<Checkbox checked={checked} disabled={checkboxAllDisabled} onChange={this.handleSelectAllRow}/>);
                selectionColumn = {
                    key: 'selection-column',
                    title: checkboxAll,
    ...
    renderColumnsDropdown(columns) {
        ...
        return columns.map((originColumn, i) => {
            ...
            if ((column.filters && column.filters.length > 0) || column.filterDropdown) {
                ...
                filterDropdown = (<FilterDropdown locale={locale} column={column} selectedKeys={colFilters} confirmFilter={this.handleFilter}/>);
            }
            if (column.sorter) {
                let isSortColumn = this.isSortColumn(column);
                if (isSortColumn) {
                    ...
                    if (sortOrder) {
                        column.className += ' rubix-table-column-sort';
                    }
                }
                const isAscend = isSortColumn && sortOrder === 'ascend';
                const isDescend = isSortColumn && sortOrder === 'descend';
                sortButton = (<div className="rubix-table-column-sorter">
            <span className={`rubix-table-column-sorter-up ${isAscend ? 'on' : 'off'}`} title="↑" onClick={() => this.toggleSortOrder('ascend', column)}>
              <Icon type="caret-up"/>
            </span>
            <span className={`rubix-table-column-sorter-down ${isDescend ? 'on' : 'off'}`} title="↓" onClick={() => this.toggleSortOrder('descend', column)}>
    ...
    renderPagination() {
        ...
        return (total > 0) ?
            <Pagination {...pagination} className={`${this.props.prefixCls}-pagination`} onChange={this.handlePageChange} total={total} size={size} current={this.getMaxCurrent(total)} onShowSizeChange={this.handleShowSizeChange}/> : null;
    }
    ...
    recursiveSort(data, sorterFn) {
        ...
        return data.sort(sorterFn).map(item => (item[childrenColumnName] ? assign({}, item, {
    ...
    render() {
        ...
        const classString = classNames({
            [`rubix-table-${this.props.size}`]: true,
            'rubix-table-bordered': this.props.bordered,
            'rubix-table-empty': !data.length,
        });
        ...
        let table = (<RcTable {...restProps} data={data} columns={columns} className={classString} expandIconColumnIndex={(columns[0] && columns[0].key === 'selection-column') ? 1 : 0} expandIconAsCell={expandIconAsCell} emptyText={() => locale.emptyText}/>);
        // if there is no pagination or no data,
        // the height of spin should decrease by half of pagination
        const paginationPatchClass = (this.hasPagination() && data && data.length !== 0)
            ? 'rubix-table-with-pagination'
            : 'rubix-table-without-pagination';
        const spinClassName = this.props.loading ? `${paginationPatchClass} rubix-table-spin-holder` : '';
        table = <Spin className={spinClassName} spinning={this.props.loading}>{table}</Spin>;
        return (<div className={`${className} clearfix`} style={style}>
        {table}
